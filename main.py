...
def find_primes(max_number=10000000, batch_size=1000000):
    current = 2
    total_primes = 0
    with tqdm(total=max_number, desc="Progress", unit=" number") as progress_bar:
        while current < max_number:
            ...
            for i in range(batch_size):
                if results[i] == 1:
                    ...
                    tqdm.write(f"Prime: {prime}, Digits: {digits}, Total Primes: {total_primes}")
            progress_bar.update(batch_size)
            current += batch_size

if __name__ == "__main__":
    find_primes(10000000000000000, 10000000000000000)